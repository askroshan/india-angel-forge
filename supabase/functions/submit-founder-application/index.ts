...
serve(async (req) => {
  ...
  try {
    ...
    const emailDomain = getEmailDomain(data.founder_email);
    const ipIdentifier = `founder_ip_${clientIP}`;
    const domainIdentifier = `founder_domain_${emailDomain}`;
    ...
    await supabaseAdmin.from("rate_limits").insert([
      { identifier: ipIdentifier, action: "founder_application" },
      { identifier: domainIdentifier, action: "founder_application" }
    ]);
